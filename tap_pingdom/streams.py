...
class Contacts(PingdomStream):
    ...
    def schema(self) -> dict[str, Any]:
        ...
        """
        # Get the base schema from OpenAPI
        base_schema = OPENAPI_SCHEMA.get_schema("ContactTargets")

        # Simplify notification_targets from complex anyOf to simple object
        # The actual data structure is an object with keys like 'email', 'sms'
        # each containing arrays of notification configs
        patches = {
            "properties": {
...
class Results(PingdomStream):
    ...
    #    response wrapper, not as a reusable component that StreamSchema
    #    can reference
    # 2. The 'checkid' property is added via post_process() from parent
