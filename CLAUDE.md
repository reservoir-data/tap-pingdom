...
1. **Type Mismatches**: The OpenAPI spec has incorrect types in several places:
...
1. **Missing Properties**: Some schemas are incomplete:

   - **Results**: Missing `probedesc` property that API actually returns

1. **Complex Types**: Some schemas use complex `anyOf` references that may not work well with Singer SDK:

   - **Contacts**: `notification_targets` uses anyOf with multiple schema refs (SMSes, Emails, APNS, AGCM)

1. **Injected Properties**: Child streams add properties from parent context:
...
- `Contacts` → `ContactTargets` (with `schema` property patches)
...
**Schema Patching with `schema` Property:**
Some streams use OpenAPI schemas but need adjustments. Instead of redefining schemas from scratch, these streams:

1. Override the `schema` property to get the base schema from OpenAPI
1. Apply patches via `apply_schema_patch()`
1. Patches can fix types, remove complex structures, or add missing properties

**Important**: Use `schema` property override (not `effective_schema`) because:

- `schema` defines what gets published to the catalog and used for validation
- `effective_schema` is only for record transformation/conformance during sync
- Tests validate records against the `schema` property
...
class MyStream(PingdomStream):
    # Don't use StreamSchema descriptor if patching
    records_jsonpath = "$.mydata[*]"

    @property
    @override
    def schema(self) -> dict[str, Any]:
        # Get base schema from OpenAPI
        base_schema = OPENAPI_SCHEMA.get_schema("MyComponent")
...
1. Verify stream schemas still match
1. Update any manual schema definitions if needed
1. Run schema evolution tests to catch breaking changes
...
1. Try using `StreamSchema(OPENAPI_SCHEMA, key="NewComponent")`
1. If the component has minor issues (wrong types, complex structures), use `effective_schema` with patches instead of manual schemas
1. Run `uv run tap-pingdom --discover > catalog.json` to verify schema generation
1. Run schema snapshot tests: `tox -e py314 -- tests/test_schema_evolution.py`
1. Compare generated schema with manual/expected schema to ensure they match
1. If successful, replace manual schema with OpenAPI + patches (if needed) and update/remove NOTE comments
...
1. Set required attributes: `name`, `path`, `primary_keys`, `replication_key`, `records_jsonpath`
1. Define schema:
   - **Preferred**: Use `StreamSchema(OPENAPI_SCHEMA, key="ComponentName")` if available
   - **If patches needed**: Override `schema` property with `apply_schema_patch()`
   - **Last resort**: Define manual schema with `th.PropertiesList` and add NOTE comment explaining why
1. Override `get_url_params()` if custom parameters needed (e.g., limit, filters)
1. Override `get_new_paginator()` if custom page size needed (default is 100)
1. Add to `STREAM_TYPES` list in `tap_pingdom/tap.py` (or comment out if requires special permissions)
1. Add schema snapshot test in `tests/test_schema_evolution.py`
...
1. Check API responses by running with `--log-level DEBUG`
1. Verify authentication token has required permissions
1. Check Pingdom API documentation at https://docs.pingdom.com
...
1. Verify the `limit` parameter in `get_url_params()` matches the paginator's `page_size`
1. Run with `--log-level DEBUG` to see offset values and record counts
1. Test the paginator's `has_more()` logic - it returns `True` if `record_count >= page_size`
1. Verify the API is returning the expected number of records per page
